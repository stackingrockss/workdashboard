...
// Dialog for adding a Granola note linked to a calendar event
...
import { createGranolaNote } from "@/lib/api/granola-notes";
import { formatDateShort } from "@/lib/format";
import type { NoteType } from "@/types/granola-note";
import type { PreselectedCalendarEvent } from "@/types/timeline";

interface AddGranolaFromCalendarDialogProps {
  open: boolean;
...
export function AddGranolaFromCalendarDialog({
  open,
  onOpenChange,
  opportunityId,
  calendarEvent,
  onSuccess,
}: AddGranolaFromCalendarDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!url.trim()) {
      toast.error("Please enter the Granola note URL");
      return;
    }
    ...
    try {
      ...
      const successMessage = transcriptText.trim()
        ? "Granola note added. Parsing transcript in background..."
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to add Granola note"
      );
    } finally {
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="granola-url">Granola Note URL *</Label>
            <Input
              id="granola-url"
              type="url"
              value={url}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Paste Granola transcript here (optional)..."
              rows={6}
              className="resize-y"
            />
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Adding..." : "Add Granola Note"}
            </Button>
          </div>
        </form>
      </DialogContent>
