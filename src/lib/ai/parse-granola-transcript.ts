/**
 * Granola Transcript Parser
 *
 * Extracts key sales insights from Granola note transcripts:
...
export interface GranolaTranscriptParsed {
  painPoints: string[];
  goals: string[];
...
/**
 * Parses a Granola transcript to extract actionable sales insights
 *
 * This function uses AI (Gemini) to analyze Granola meeting transcripts and extract:
 ...
 *
 * @param transcriptText - The full transcript text from Granola (minimum 100 characters)
 ...
 * @example
 * ```typescript
 * const result = await parseGranolaTranscript(transcriptText, "Acme Corp");
 ...
 *
 * @see {@link GranolaParseResult} for return type
 * @see {@link GranolaTranscriptParsed} for data structure
 * @see {@link PersonExtracted} for people data structure
 */
export async function parseGranolaTranscript(
  transcriptText: string,
  userOrganizationName?: string
): Promise<GranolaParseResult> {
  try {
    // Validate input
    ...
    const prompt = `Analyze the following Granola meeting transcript and extract pain points, goals, people, and next steps.
...
Return your analysis as JSON only.`;
    ...
    // Parse JSON response
    let parsedData: GranolaTranscriptParsed;
    try {
