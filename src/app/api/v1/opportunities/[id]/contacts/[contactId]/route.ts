...
export async function GET(
  ...
  try {
    ...
    if (!contact || contact.opportunityId !== id) {
...
// PATCH /api/v1/opportunities/[id]/contacts/[contactId] - Update a contact
export async function PATCH(
  ...
  try {
    ...
    if (!existingContact || existingContact.opportunityId !== id) {
      return NextResponse.json(
        { error: "Contact not found" },
        { status: 404 }
      );
    ...
    // If managerId is being updated, verify the manager exists and belongs to the same opportunity
...
// DELETE /api/v1/opportunities/[id]/contacts/[contactId] - Delete a contact
